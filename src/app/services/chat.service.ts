...
export class ChatService {
  ...
  sendWelcomeMessages() {
    const startMessage: ChatMessageModel = {
      messageText:
        "Welcome to the exhibition. This ArtBot Guide will introduce the Nias/Indies masks and their history.",
      sentByBot: true,
    };

    const disclaimerMessage: ChatMessageModel = {
      messageText:
        "Despite the best efforts of the researchers to include the perspective of the locals, limited written resources were available on this topic. Therefore, please note that the information in the chatbot mostly relies on Western sources.",
      sentByBot: true,
    };

    const sourcesMessage: ChatMessageModel = {
      messageText:
        "If you want to learn more about the Nias/Indies masks and physical anthropology, you can click on the source’s title on the top left of each message to access the original source.",
      sentByBot: true,
    };
  ...
  sendMessages(messages: ChatMessageModel[]) {
    ...
    for (let messageIdx = 0; messageIdx < messages.length; messageIdx++) {
      ...
      let timeToTypeMessage =
        environment.timeDelayPerKeystroke * message.messageText.length;
      ...
      setTimeout(() => {
        const sentMessage: ChatMessageModel = JSON.parse(
