...
export class TimeSliderComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.yScale = D3.scaleTime()
      .domain([this.maxDate, this.minDate])
      .range([0, this.height]);
    this.yScale2 = D3.scaleTime()
      .domain([new Date(2000, 1, 1), new Date(1900, 1, 1)])
      .range([0, this.height]);
    ...
    // A color scale
    let colorScale = D3.scaleSequential(D3.interpolateViridis).domain([
      this.height,
      0,
    ]);
    ...
    // animate button
    const playButton = this._svg
      .append("g")
      .attr(
        "transform",
        "translate(" + 0 + "," + (this.height + buttonYSize) + ")"
      )
    ...
    // reset button
    const resetFilterButton = this._svg
      .append("g")
  ...
  getTimePeriodFromSlider(): Array<Date> {
    ...
    let startDate_yPosition: any = D3.select("g.brush rect.handle--s").attr(
      "y"
    );
  ...
  isBrushNull(): boolean {
    if (D3.select("g.brush rect.handle--n").attr("y")) {
      return true;
    }
    return false;
  }
