...
export class CubeComponent implements AfterViewInit {
  // Canvases
  @ViewChild("webGLCanvas", { static: false }) webGLContainer: ElementRef;
  ...
  // inject google
  constructor(
    private google: GoogleDriveProvider,
    private localFiles: LocalFileLoaderService,
  ...
  ngAfterViewInit() {
    ...
    this.gBtn.nativeElement.className = "btn  btn-primary";
    this.sBtn.nativeElement.className = "btn  btn-primary";
    this.nBtn.nativeElement.className = "btn  btn-primary";
   ...
   */
  initScene = async () => {
    // TODO: Remove this dirty hack to properly retrieve webGLContainer dynamic width and height
    const waitForCanvasWidthAndHeightToUpdate = new Promise((res, rej) => {
      setTimeout(
        () => res("Done waiting for canvas width and height to update."),
        500
      );
    });
    await waitForCanvasWidthAndHeightToUpdate;
    ...
    this.webGLRenderer = new THREE.WebGLRenderer({
      canvas: this.webGLContainer.nativeElement as HTMLCanvasElement,
      alpha: true,
    });
    this.webGLRenderer.setSize(WIDTH, HEIGHT);
    this.webGLRenderer.setClearColor(0xffffff, 0);

    // Fix for overlapping webgl
    ...
    // Fix for overlapping component css
    ...
    this.camera.lookAt(
      this.webGLScene.position.x,
      this.webGLScene.position.y,
      this.webGLScene.position.z
    );
    ...
    this.webGLContainer.nativeElement.addEventListener("click", ($event) => {
      ...
      if (foundItem) {
        this.previewItem = {
          ...
          categories: [
            foundItem.category_1,
            foundItem.category_2,
            foundItem.category_3,
            foundItem.category_4,
            foundItem.category_5,
          ],
  ...
  getClickedItem = ($event) => {
    ...
    let foundItem = this.gCube.onClick(
      $event,
      this.tooltip,
      this.webGLContainer.nativeElement
    );
    if (!foundItem) {
      foundItem = this.sCube.onClick(
        $event,
        this.tooltip,
        this.webGLContainer.nativeElement
      );
  ...
  getPrevious(): void {
    ...
    const foundIdx = this.dataManager.data
      .map((d: any) => {
        return d.id;
      })
    ...
    const foundItem =
      this.dataManager.data[(foundIdx - 1) % this.dataManager.data.length];
  ...
  initGUI = () => {
    ...
    this.gui.pCubeConfigEmitter.on("change", (change: any) => {
      ...
      if (change.backgroundColor) {
        this.compRef.nativeElement.ownerDocument.body.style.backgroundColor =
      ...
      if (change.nodeColor) {
        this.showColorCodingLegend =
          change.nodeColor !== "categorical" ? false : true;
  ...
  transitionSICamera(): void {
    ...
    this.restoreCamera(
      this.camToSave.position,
      this.camToSave.rotation,
      this.camToSave.controlCenter
    );
  ...
  positionCamera = (): void => {
    ...
    const tweenPos = new TWEEN.Tween(this.camera.position);
    const tweenLookAt = new TWEEN.Tween(
      camLookAt.applyQuaternion(this.camera.quaternion)
    );
  ...
  setCubeView(view: string): void {
    switch (view) {
      case "GEO_CUBE":
        ...
        this.gBtn.nativeElement.className = this.gCube.cubeToggle
          ? "btn  btn-primary"
          : "btn  btn-dark";
        break;
      case "SET_CUBE":
        this.sCube.toggleDisplayCube();
        this.sBtn.nativeElement.className = this.sCube.cubeToggle
          ? "btn  btn-primary"
          : "btn  btn-dark";
        break;
      case "NET_CUBE":
        this.nCube.toggleDisplayCube();
        this.nBtn.nativeElement.className = this.nCube.cubeToggle
          ? "btn  btn-primary"
          : "btn  btn-dark";
        break;
  ...
  applyFilter(): void {
    this.gCube.filterData(
      this.currentlySelectedCategory,
      this.currentlySelectedDateExtent[0],
      this.currentlySelectedDateExtent[1]
    );
    this.sCube.filterData(
      this.currentlySelectedCategory,
      this.currentlySelectedDateExtent[0],
      this.currentlySelectedDateExtent[1]
    );
    this.nCube.filterData(
      this.currentlySelectedCategory,
      this.currentlySelectedDateExtent[0],
      this.currentlySelectedDateExtent[1]
    );
   ...
   */
  getMinDate(): Date {
    return this.dataManager.getMinDate();
  }
   ...
   */
  getMaxDate(): Date {
    return this.dataManager.getMaxDate();
  }
