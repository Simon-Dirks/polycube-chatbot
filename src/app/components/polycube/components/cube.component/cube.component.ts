...
export class CubeComponent implements AfterViewInit {
    ...
    // inject google
    constructor(private google: GoogleDriveProvider, private localFiles: LocalFileLoaderService, private gps: GraphPositionService, private dataManager: DataManager, private compRef: ElementRef) {
    ...
    initScene = async () => {
        ...
        const waitForCanvasWidthAndHeightToUpdate = new Promise((res, rej) => {
            setTimeout(() => res('Done waiting for canvas width and height to update.'), 500);
        });
    ...
    getClickedItem = ($event) => {
        ...
        if (!foundItem) {
            foundItem = this.sCube.onClick($event, this.tooltip, this.webGLContainer.nativeElement);
        }
        if (!foundItem) {
            foundItem = this.nCube.onClick($event, this.tooltip, this.webGLContainer.nativeElement);
        }
    ...
    setCubeView(view: string): void {
        switch (view) {
            case 'GEO_CUBE':
                ...
                this.gBtn.nativeElement.className = this.gCube.cubeToggle ? 'btn  btn-primary' : 'btn  btn-dark';
                break;
            case 'SET_CUBE':
                this.sCube.toggleDisplayCube();
                this.sBtn.nativeElement.className = this.sCube.cubeToggle ? 'btn  btn-primary' : 'btn  btn-dark';
                break;
            case 'NET_CUBE':
                this.nCube.toggleDisplayCube();
                this.nBtn.nativeElement.className = this.nCube.cubeToggle ? 'btn  btn-primary' : 'btn  btn-dark';
                break;
