...
export class SetCube implements PolyCube {
  ...
  updateSetCube(
    segs: number = this.dm.timeRange.length,
    initial: boolean = false,
    layout: string = "pack"
  ): void {
    // pass object parameter to function
    ...
    this.dm.data.forEach((d: any) => {
      ...
      d.groupDate = moment(
        this.dm.getCustomTimeQuantile(d.date_time, segs),
        "YYYY"
      ).toDate();
    });
    ...
    groupedData.forEach((timeLayer: any, i: number) => {
      ...
      timeLayer.values.forEach((category: any) => {
        // slices group
        ...
        const phyllotaxis = this.getPhyllotaxis(
          circle.position.x,
          circle.position.z,
          rad,
          category.values
        ...
        phyllotaxis.forEach((points) => {
          // points group

          // this.updateColorCoding('temporal');
          const material2 = new THREE.MeshBasicMaterial({
            color: this.getCurrentColor(points),
          }); // FIXME: Color not found on SI
  ...
  forceClusterGraph(): void {
    ...
    d3Nodes.map(function (data) {
      ...
      let i = data.cluster,
        ...
        d = {
          ...
          radius: r,
          x:
            Math.cos((i / m) * 2 * Math.PI) * 150 +
            CUBE_CONFIG.WIDTH / 2 +
            Math.random(),
          y:
            Math.sin((i / m) * 2 * Math.PI) * 150 +
            CUBE_CONFIG.HEIGHT / 2 +
            Math.random(),
        };
    ...
    const layout = D3.forceSimulation()
      ...
      .force(
        "cluster",
        forceCluster().centers(function (d) {
    ...
    function layoutTick() {
      if (
        cntTicks++ > state.coolDownTicks ||
        new Date().getTime() - startTickTime > state.coolDownTime
      ) {
  ...
  clearLabels(): void {
    ...
    removed.forEach((r: THREE.CSS3DObject) => {
  ...
  clearSetLabels(): void {
    ...
    D3.selectAll(".set-label").style("display", "none");
  }

  updateLayout(layout: string): void {
    const segs = this.dm.timeRange.length;
    if (layout === "cluster") {
      // just for testing cluster force in UI
      // console.log('cluster')
  ...
  getPackLayout() {
    ...
    const layout = vLayout(vRoot).children.map((d: any) => {
      return {
        cat: d.data.Category,
        x: d.x - CUBE_CONFIG.WIDTH / 2,
        y: d.y - CUBE_CONFIG.HEIGHT / 2,
        count: d.value,
        r: d.r,
      };
    });
  ...
  getPhyllotaxis(centerX: number, centerY: number, radius: number, data: any) {
    data.sort((a: any, b: any) => {
      (a = Date.parse(a.date_time)), (b = Date.parse(b.date_time));
  ...
  geometryConvex(group = "Identification photographs") {
    ...
    vertices.forEach((d, i) => {
      ...
      if (i !== vertices.length - 1) {
        // if to deal with last component structure
