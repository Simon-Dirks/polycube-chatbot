...
export class NetCube implements PolyCube {
      ...
      this.dm.data.forEach((target: any) => {
        ...
        target.target_nodes.forEach((targetNode: any) => {
          if (targetNode === source.id) {
            // if target of target is same as source - incoming edge
    ...
    this.cubeGroupGL.children.forEach((child: THREE.Group, i) => {
      ...
      child.children.forEach((grandChild: any) => {
        ...
        grandChild.position.y =
          time === "aggregated"
            ? 0
            : this.timeLinearScale(grandChild.data.date_time) - sliceOffsetY;
      });
    });

    this.currentTimeSetting === "aggregated"
      ? this.showCubeLinks_aggregated()
      : this.showCubeLinks_absolute();
  }
  ...
  updateColorCoding(encoding: string): void {
    ...
    switch (encoding) {
      case "categorical":
        ...
        break;
      case "temporal":
        this.colors = D3.scaleSequential(D3.interpolateViridis).domain([
          this.dm.getMinDate(),
          this.dm.getMaxDate(),
        ]);
        break;
      ...
      default: //D3.scaleOrdinal(D3.schemePaired);
        this.colors = this.dm.colors;
        break;
    }
  }
  ...
  areBothDatesWithinDateInterval(
    startDate: Date,
    endDate: Date,
    dates: Array<Date>
  ): boolean {
    let isFirstDate =
      moment(dates[0]) >= moment(startDate) &&
      moment(dates[0]) <= moment(endDate);
    let isSecondDate =
      moment(dates[1]) >= moment(startDate) &&
      moment(dates[1]) <= moment(endDate);
  ...
  areBothNodesFromSameTargetCategory(link: any, category: string): boolean {
    ...
    if (
      this.areBothSameNodeCategory(bothNodeCategories) &&
  ...
  isDateWithinInterval(
    ...
    return (
      moment(pointDate) >= moment(startDate) &&
      moment(pointDate) <= moment(endDate)
    );
  }
  ...
  transitionSTC(): void {
    ...
    this.currentTimeSetting === "aggregated"
      ? this.showCubeLinks_aggregated()
      : this.showCubeLinks_absolute(); // get back setting
    this.showBottomLayer();
  ...
  getCurrentColor(object: THREE.Object3D): string {
    switch (this.colorCoding) {
      case "categorical":
        return this.colors(object.data.category_1);
      case "temporal":
        return this.colors(object.data.date_time);
      case "monochrome":
        return "#cc1414";
      default:
        return this.colors(object.data.category_1);
  ...
  resetSelection(gray: boolean = false): void {
    this.cubeGroupGL.children.forEach((child: any) => {
      ...
      child.children.forEach((grandChild: any) => {
        ...
        grandChild.material.color.set(
          gray ? "#b5b5b5" : this.getCurrentColor(grandChild)
        );
      });
    });
  }
  ...
  getNormalizedPositionWithChargeById(id: string): any {
    ...
    if (pos_map.has(`${id}`)) {
      normalized_x =
        ((pos_map.get(`${id}`).x * CUBE_CONFIG.WIDTH) /
          Math.abs(pos_dim.max_x - pos_dim.min_x)) *
          this.chargeFactor +
        CUBE_CONFIG.WIDTH / 2;
      normalized_z =
        ((pos_map.get(`${id}`).y * CUBE_CONFIG.WIDTH) /
          Math.abs(pos_dim.max_y - pos_dim.min_y)) *
          this.chargeFactor +
        CUBE_CONFIG.WIDTH / 2;
    }
  ...
  createNodes(): void {
    ...
    this.degreeScale = D3.scaleLinear()
    ...
    for (let i = 0; i < this.dm.data.length; i++) {
      ...
      let material = new THREE.MeshBasicMaterial({
        color: this.colors(dataItem.category_1),
      });
  ...
  createLinks(): void {
    ...
    for (let i = 0; i < this.dm.data.length; i++) {
      ...
      let sourceNode_position = this.getNormalizedPositionById(
        `${dataItem.id}`
      );
      if (sourceNode_position === null) {
        continue;
      }

      sourceNode_position.y = this.getTimeSliceByDate(
        dataItem.date_time
      ).position.y;
      ...
      for (let a = 0; a < this.linksPerNode; a++) {
        ...
        if (this.doesTargetNodeHasPosition(targetId)) {
          //STC aggregated
          let line_forSTC_aggregated = this.createLineForSTC_aggregated(
            dataItem,
            sourceNode_position,
            a
          );
          ...
          //STC absolute
          let line_forSTC_absolute = this.createLineForSTC_absolute(
            dataItem,
            sourceNode_position,
            a
          );
          ...
          //SI
          let line_forSI = this.createLineForSI(
            dataItem,
            sourceNode_position,
            a
          );
          ...
          //JP
          let line_forJP = this.createLineForJP(
            dataItem,
            sourceNode_position,
            a
          );
  ...
  createLineForJP(dataItem, sourceNode_position, targetIndex) {
    ...
    targetNode_position.y = this.getTimeSliceByDate(
      this.dm.dataMap[targetId].date_time
    ).position.y;
  ...
  areSourceAndTargetInTheSameSlice(
    sourceId: string,
    targetId: string
  ): boolean {
    if (
      this.getTimeSliceById(sourceId).name ===
      this.getTimeSliceById(targetId).name
    )
      return true;
    return false;
  }
  ...
  createLineForSTC_absolute(dataItem, sourceNode_position, targetIndex) {
    ...
    let targetPosition: THREE.Vector3 = {
      x: targetNode.x,
      y: this.timeLinearScale(
        moment(`${this.dm.dataMap[targetId].date_time}`).toDate()
      ),
      z: targetNode.z,
    };
  ...
  createLineGeometry(
    sourceNode_position: THREE.Vector3,
    targetNode_position: THREE.Vector3
  ) {
  ...
  updateSlices(): void {
    ...
    for (let i = 0; i < this.dm.timeRange.length; i++) {
      ...
      slice.position.set(
        CUBE_CONFIG.WIDTH / 2,
        i * vertOffset - CUBE_CONFIG.WIDTH / 2,
        CUBE_CONFIG.WIDTH / 2
      );
  ...
  createSlices(): void {
    ...
    for (let i = 0; i < this.dm.timeRange.length; i++) {
      ...
      slice.position.set(
        CUBE_CONFIG.WIDTH / 2,
        this.timeLinearScale(moment(`${slice.name}`).toDate()),
        CUBE_CONFIG.WIDTH / 2
      );
      slice.index = i;
      plane.position.set(0, 0, 0);
