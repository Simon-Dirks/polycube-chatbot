...
export class GeoCube implements PolyCube {
  ...
  constructor(
    ...
    (mapboxgl as typeof mapboxgl).accessToken =
      environment.mapbox.polycubeAccessToken;
  ...
  showMapImages(): void {
    this.hiddenLabels.forEach((image: THREE.CSS3DObject) => {
      this.cubeGroupCSS.add(image);
    });
  ...
  hideMapImages(): void {
    this.cubeGroupCSS.traverse((object: THREE.CSS3DObject) => {
      if (object.name.includes("MAP_CONTAINER"))
        this.hiddenMapImages.push(object);
    });
    ...
    this.hiddenLabels.forEach((image: THREE.CSS3DObject) => {
      this.cubeGroupCSS.remove(image);
    });
  }
  ...
  updateDataPoints(): void {
    ...
    for (let i = 0; i < this.dm.data.length; i++) {
      ...
      let cubeCoords = this.map.project(
        new mapboxgl.LngLat(dataItem.longitude, dataItem.latitude)
      );
  ...
  assembleData(): void {
    ...
    this.dm.data.forEach((d: any) => {
      bounds.extend(new mapboxgl.LngLat(d.longitude, d.latitude));
    });
        ...
        switch (encoding) {
          case "categorical":
            grandChild.material.color.set(
              this.colors(grandChild.data.category_1)
            );
            break;
          case "temporal":
            grandChild.material.color.set(
              this.colors(grandChild.data.date_time)
            );
            break;
          case "monochrome":
  ...
  jitterPoint(): void {
    this.cubeGroupGL.children.forEach((child: THREE.Group) => {
      ...
      child.children.forEach((grandChild: any) => {
        ...
        let sourceCoords = {
          x: grandChild.originalCoordinates
            ? grandChild.originalCoordinates.x
            : grandChild.position.x,
          y: grandChild.originalCoordinates
            ? grandChild.originalCoordinates.y
            : grandChild.position.y,
          z: grandChild.originalCoordinates
            ? grandChild.originalCoordinates.z
            : grandChild.position.z,
        };

        let targetCoords = {
          x: grandChild.originalCoordinates
            ? grandChild.originalCoordinates.x + xJitter
            : grandChild.position.x + xJitter,
          y: grandChild.originalCoordinates
            ? grandChild.originalCoordinates.y
            : grandChild.position.y,
          z: grandChild.originalCoordinates
            ? grandChild.originalCoordinates.z + zJitter
            : grandChild.position.z + zJitter,
        };

        if (!grandChild.originalCoordinates)
          grandChild.originalCoordinates = new THREE.Vector3(
            sourceCoords.x,
            sourceCoords.y,
            sourceCoords.z
          );
  ...
  filterData(cat: string, start: Date, end: Date): void {
    this.cubeGroupGL.children.forEach((child: THREE.Group) => {
      ...
      child.children.forEach((grandChild: any) => {
        ...
        if (
          !(
            this.dateWithinInterval(start, end, grandChild.data.date_time) &&
            (cat === "" ? true : grandChild.data.category_1 === cat)
          )
        ) {
  ...
  transitionJP(): void {
    ...
    this.slices.forEach((slice: THREE.Group, i: number) => {
      ...
      mapClone.position.set(
        new THREE.Vector3(slice.position.x, slice.position.y, slice.position.z)
      );
